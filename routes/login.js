    ...
    if (err || !myUser)
      ...
      User.findOneAndUpdate({
        ...
        if (req.body.isBlogger) {
          User.update({ isAdmin: true, isActive: true }, { $push: { inbox: [{kind:"blog", title: "Request for blogger", content: "I want to be a blogger", sender: user.userName, date: Date.now(), isRead: false, isConfirm: false }] }, $inc: { inboxCount: 1 } },
            function (err, admin) {
              if (!err && admin) {
